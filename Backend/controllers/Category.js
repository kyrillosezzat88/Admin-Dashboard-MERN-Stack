...
const Create = async (req, res) => {
  try {
    ...
    if (getCategory)
      return res.status(400).json({ message: "Category created before!" });
    let uploadimage;
    if (req.file) {
    ...
    let newCategory = new CategoryModel({
      ...req.body,
      mainImage: uploadimage ? uploadimage.secure_url : "",
    });
...
const update = async (req, res) => {
  try {
    ...
    let updatedCategory = await CategoryModel.findByIdAndUpdate(
      id,
      uploadimage
        ? { ...req.body, mainImage: uploadimage.secure_url }
        : req.body,
      { new: true }
    );
    if (!updatedCategory)
    ...
    return res
      .status(200)
      .json({ message: "updated successgully", data: updatedCategory });
  } catch (error) {
    return res.status(500).json(error.message);
  }
};
...
const Delete = async (req, res) => {
  try {
    ...
    if (products.length) {
      products.forEach(async (pro) => await pro.remove());
    }
